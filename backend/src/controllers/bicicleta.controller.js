...
export async function postBicicleta(req, res) {
  try {
    ...
    if (!req.body.marca || !marcaRegex.test(req.body.marca)) {
      ...
      return handleErrorClient(res, 400, "La marca/modelo contiene caracteres no válidos.");
    }
    if (req.body.marca.length > 50) {
      await fs.unlink(req.file.path);
      return handleErrorClient(res, 400, "La marca/modelo es demasiado larga (máx 50 caracteres).");
    }
